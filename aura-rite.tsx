...
import { LayoutGrid, Trophy, Settings as SettingsIcon, BarChart3, FastForward, Pause, Lock, X, RotateCcw, CheckCircle2, Circle } from "lucide-react";
...
const TIERS = [
  ...
  { name: "Transcendent", color: "#000000" },
];
...
const AURAS = [
  ...
  { id: "omega", name: "Omega", chance: 4000000000, tier: 7 },
...
function effectFor(tier) {
  ...
  return { rings: 4, duration: 1900 }; // Transcendent
}
...
export default function App() {
  ...
  /* used when something *external* interrupts a roll (nav buttons, reset) —
     also stops the auto-roll loop, unlike clearRollTimers above */
  ...
  /* any other button interrupts an in-progress roll and switches view */
  const goToView = useCallback(
    (name) => {
      if (spinningRef.current) cancelRoll();
  ...
  return (
    <div className="ar-root">
      <style>{`
        ...
        @keyframes ar-lucky-pulse { 0%, 100% { opacity: 0.65; } 50% { opacity: 1; } }
        .ar-roll-charge-track { position: absolute; left: 0; right: 0; bottom: 0; height: 4px; background: rgba(255,255,255,0.16); }
        .ar-roll-charge-fill { display: block; height: 100%; background: #FFD60A; transition: width 0.3s ease; }
        ...
        @keyframes ar-lucky-glow {
          ...
          50% { box-shadow: 0 0 20px 5px rgba(255,214,10,0.55); }
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          .ar-ring, .ar-reveal-text.ar-pop, .ar-toast { animation: none !important; }
      ...
      <div className="ar-stage" style={{ transform: zoomed ? `scale(${PEAK_SCALE})` : "scale(1)", transitionProperty: "transform", transitionDuration: `${zoomDuration}ms`, transitionTimingFunction: zoomed ? "linear" : "ease-out" }}>
        <header className="ar-header">
          <div className="ar-header-left">
            <div className="ar-badge">{bestAura && <AuraVisual aura={bestAura} size={36} />}</div>
            <div className="ar-rolls-caption">{data.totalRolls.toLocaleString()} rolls so far</div>
          </div>
          <nav className="ar-nav">
            <button className={`ar-icon-btn ${view === "stats" ? "active" : ""}`} onClick={() => goToView("stats")} aria-label="Stats">
              <BarChart3 size={17} />
            </button>
            <button className={`ar-icon-btn ${view === "collection" ? "active" : ""}`} onClick={() => goToView("collection")} aria-label="Collection">
              <LayoutGrid size={17} />
            </button>
            <button className={`ar-icon-btn ${view === "achievements" ? "active" : ""}`} onClick={() => goToView("achievements")} aria-label="Achievements">
              <Trophy size={17} />
            </button>
            <button className={`ar-icon-btn ${view === "settings" ? "active" : ""}`} onClick={() => goToView("settings")} aria-label="Settings">
        ...
        <main className="ar-main">
          <div className="ar-panel">
            {view === "roll" && (
              <div className="ar-reveal-area">
                {revealAuraId && <AuraVisual aura={AURAS.find((a) => a.id === revealAuraId)} size={150} />}
                {burst &&
          ...
          <div className="ar-action-row">
            <button className={`ar-roll-btn ${showLucky ? "lucky-glow" : ""}`} onClick={doRoll} disabled={spinning}>
              <span className="ar-roll-label">Roll</span>
              {showLucky && <span className="ar-roll-lucky-tag">Lucky ×{LUCKY_MULTIPLIER}</span>}
              <span className="ar-roll-charge-track">
                <span className="ar-roll-charge-fill" style={{ width: `${luckyChargeRatio * 100}%` }} />
              </span>
            </button>
            <button
          ...
          <button className={`ar-inventory-btn ${view === "collection" ? "active" : ""}`} onClick={() => goToView("collection")}>
            <LayoutGrid size={17} />
            Inventory
          </button>
        </main>
